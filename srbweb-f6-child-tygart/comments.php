...
<?php //Added to grab first name of commenter and last initial - reference: https://wordpress.org/support/topic/user-first-name
	...
	$commenter_lname =  substr(get_user_meta($commenter_id,'last_name', true), 0, 1);
...
?>
		...
		<section id="respond">
			...
			<p><?php printf( __('You must <a href="%s">log in</a> to post a comment.', 'srbweb-foundation'), wp_login_url( get_permalink() ) ); ?></p>
			<?php else : ?>
			...
			<!--<p>
				...
				<p><input name="submit" class="button" type="submit" id="submit" tabindex="5" value="<?php esc_attr_e('Submit Comment', 'srbweb-foundation'); ?>"></p>
